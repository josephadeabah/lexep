...
export default function MentorshipPage() {
  ...
  const recommended = useAsync(() => (role === "learner" ? api.recommendedMentors() : Promise.resolve([])), [role]);
  ...
  return (
    <div className="flex flex-col gap-lg">
      ...
      {role === "learner" && recommended.data && recommended.data.length > 0 && (
        <div>
          ...
          <div className="grid gap-md sm:grid-cols-2 lg:grid-cols-3">
            {recommended.data.slice(0, 3).map((mentor) => (
      ...
      <div>
        {role === "learner" && recommended.data && recommended.data.length > 0 && (
          <p className="mb-3 text-headline-md text-on-background">All Mentors</p>
        )}
        <div className="grid gap-md sm:grid-cols-2 lg:grid-cols-3">
          {mentors.isLoading ? (
