...
function NotificationsTab() {
  ...
  const EMAIL_OPTIONS: { key: keyof typeof prefs; label: string; description: string }[] = [
    { key: "new_internship_alerts", label: "New Internship Alerts", description: "Get notified when opportunities matching your profile are posted." },
    { key: "mentorship_requests", label: "Mentorship Requests", description: "Receive an email when a mentor responds to your application." },
    { key: "weekly_digests", label: "Weekly Digests", description: "A weekly summary of your progress and new resources." },
  ];
  const PUSH_OPTIONS: { key: keyof typeof prefs; label: string; description: string }[] = [
    { key: "interview_reminders", label: "Interview Reminders", description: "Important alerts for upcoming scheduled interviews." },
    { key: "message_alerts", label: "Message Alerts", description: "Instant notifications when you receive a direct message." },
  ];
    ...
    <Card>
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-label-sm text-on-surface-variant">Control how and when you receive updates.</p>
        </div>
      </div>
...
function PrivacyTab() {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await api.updatePrivacy({ profile_visibility: visibility, share_anonymized_data: shareAnonymizedData });
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
    } finally {
      setSaving(false);
  ...
  const VISIBILITY_OPTIONS: { id: typeof visibility; label: string; description: string }[] = [
    { id: "public", label: "Public", description: "Anyone in the Architect Portal can find and view your profile." },
    { id: "mentors_only", label: "Mentors Only", description: "Only verified mentors and institutional partners can view your details." },
    { id: "private", label: "Private", description: "Your profile is hidden from search and directories." },
  ];

  return (
    <Card>
      <h2 className="text-headline-md text-on-background">Profile Visibility</h2>
      <p className="text-label-sm text-on-surface-variant">Control who can see your profile and academic achievements within the network.</p>
      ...
      <h2 className="mt-lg text-headline-md text-on-background border-t border-outline-variant/40 pt-md">Data Sharing</h2>
      <p className="text-label-sm text-on-surface-variant">Manage how your data is used to improve our services and programs.</p>
      <label className="mt-3 flex cursor-pointer items-start gap-3 rounded-md bg-surface-container-low p-4">
        <Checkbox checked={shareAnonymizedData} onChange={(e) => setShareAnonymizedData(e.target.checked)} />
        <span>
          <span className="block text-body-md text-on-surface">Share anonymized data with educational partners</span>
          <span className="block text-label-sm text-on-surface-variant">
            We use this data to secure more grants and opportunities for the community. Your personal identity remains hidden.
          </span>
        </span>
      </label>

      <h2 className="mt-lg text-headline-md text-on-background border-t border-outline-variant/40 pt-md">Account Security</h2>
      <p className="text-label-sm text-on-surface-variant">Enhance the protection of your professional account.</p>
      <div className="mt-3 flex items-center justify-between rounded-md border border-outline-variant p-4">
        <span className="flex items-center gap-3">
          ...
          <span>
            <span className="block text-body-md text-on-surface">Two-Factor Authentication (2FA)</span>
            <span className="block text-label-sm text-on-surface-variant">Add an extra layer of security by requiring a code from your mobile device upon login.</span>
          </span>
        </span>
  ...
  return (
    <div>
      <div className="mb-lg">
        ...
        <p className="mt-1 text-body-md text-on-surface-variant">Manage your account preferences, notifications, and security settings.</p>
      </div>
