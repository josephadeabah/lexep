...
from app.integrations.payments.base import InitializedTransaction, PaymentProvider, VerifiedTransaction
...
class PaystackProvider(PaymentProvider):
    ...
    def initialize_transaction(
        ...
        with httpx.Client(timeout=15) as client:
            res = client.post(f"{PAYSTACK_BASE_URL}/transaction/initialize", json=payload, headers=self._headers())
            res.raise_for_status()
            data = res.json()["data"]
        return InitializedTransaction(
            reference=data["reference"], authorization_url=data["authorization_url"], provider="paystack"
        )

    def verify_transaction(self, reference: str) -> VerifiedTransaction:
        with httpx.Client(timeout=15) as client:
            res = client.get(f"{PAYSTACK_BASE_URL}/transaction/verify/{reference}", headers=self._headers())
            res.raise_for_status()
            data = res.json()["data"]
