...
class MockPaymentProvider(PaymentProvider):
    def initialize_transaction(
        ...
        return InitializedTransaction(reference=reference, authorization_url=None, provider="mock")

    def verify_transaction(self, reference: str) -> VerifiedTransaction:
        return VerifiedTransaction(
            reference=reference, status="success", amount=0, currency="USD", provider="mock", metadata={}
        )
